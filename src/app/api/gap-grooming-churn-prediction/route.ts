...
// AI gap: Grooming Churn Prediction (petGrooming)
...
async function callLLM(systemPrompt: string, userMsg: string, maxTokens = 1400) {
  ...
  const r = await fetch('https://openrouter.ai/api/v1/chat/completions', {
    method: 'POST',
    headers: { Authorization: 'Bearer ' + OPENROUTER_API_KEY, 'Content-Type': 'application/json', 'HTTP-Referer': 'http://localhost:3000', 'X-Title': 'petGrooming Gap grooming-churn-prediction' },
...
export async function POST(req: NextRequest) {
  try {
    ...
    const sys = "You predict client churn for grooming services based on appointment cadence and reminder responses.";
    const user = `Body: ${JSON.stringify(body).slice(0, 4000)}`;
    const out = await callLLM(sys, user);
    gapFeatures.push({ at: new Date().toISOString(), slug: 'grooming-churn-prediction', payload: { keys: Object.keys(body) } });
    return NextResponse.json({ risk: out });
  } catch (e: any) {
...
export async function GET() {
  return NextResponse.json({ feature: 'grooming-churn-prediction', events: gapFeatures.filter(r => r.slug === 'grooming-churn-prediction').length });
}
