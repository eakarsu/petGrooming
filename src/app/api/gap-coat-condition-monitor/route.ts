...
// AI gap: Coat-Condition Monitor (petGrooming)
...
async function callLLM(systemPrompt: string, userMsg: string, maxTokens = 1400) {
  ...
  const r = await fetch('https://openrouter.ai/api/v1/chat/completions', {
    method: 'POST',
    headers: { Authorization: 'Bearer ' + OPENROUTER_API_KEY, 'Content-Type': 'application/json', 'HTTP-Referer': 'http://localhost:3000', 'X-Title': 'petGrooming Gap coat-condition-monitor' },
...
export async function POST(req: NextRequest) {
  try {
    ...
    const sys = "You analyze coat-condition photos for changes over time. Identify concerning changes.";
    const user = `Body: ${JSON.stringify(body).slice(0, 4000)}`;
    const out = await callLLM(sys, user);
    gapFeatures.push({ at: new Date().toISOString(), slug: 'coat-condition-monitor', payload: { keys: Object.keys(body) } });
    return NextResponse.json({ analysis: out });
  } catch (e: any) {
...
export async function GET() {
  return NextResponse.json({ feature: 'coat-condition-monitor', events: gapFeatures.filter(r => r.slug === 'coat-condition-monitor').length });
}
