...
// AI gap: Historical-Driven Grooming Time Estimator (petGrooming)
...
async function callLLM(systemPrompt: string, userMsg: string, maxTokens = 1400) {
  ...
  const r = await fetch('https://openrouter.ai/api/v1/chat/completions', {
    method: 'POST',
    headers: { Authorization: 'Bearer ' + OPENROUTER_API_KEY, 'Content-Type': 'application/json', 'HTTP-Referer': 'http://localhost:3000', 'X-Title': 'petGrooming Gap grooming-time-estimator' },
...
export async function POST(req: NextRequest) {
  try {
    ...
    const sys = "You estimate grooming time based on breed, coat condition, prior history, and groomer.";
    const user = `Body: ${JSON.stringify(body).slice(0, 4000)}`;
    const out = await callLLM(sys, user);
    gapFeatures.push({ at: new Date().toISOString(), slug: 'grooming-time-estimator', payload: { keys: Object.keys(body) } });
...
export async function GET() {
  return NextResponse.json({ feature: 'grooming-time-estimator', events: gapFeatures.filter(r => r.slug === 'grooming-time-estimator').length });
}
